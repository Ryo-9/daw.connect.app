...
2026-09-02 時点で、初期ドキュメント（PR #2）、モックデータによる主要 UI（PR #3）、非永続インタラクション（PR #4）は main へマージ済みです。現在の操作データはブラウザ内の一時状態で、DB、API、認証、AWS、自動テスト、CI は未実装です。

次に検討する候補は以下です。順序や着手日は確定事項ではなく、担当と変更範囲を確認してから選びます。

| 候補 | ID | 内容 | 依存・注意 |
| --- | --- | --- | --- |
| 1 | BRIDGE-005 | テスト方針の作成 | 実装やパッケージ追加を含めず、対象と実行タイミングを決める |
| 2 | BRIDGE-006 | PR ごとの lint / build CI | CI サービスと必須条件を合意してから設定する |
| 3 | BRIDGE-008 | 主要 UI の自動テスト追加 | BRIDGE-005 で対象とツールを決めた後に着手する |
| 4 | SURFACE-015 | 主要画面のモバイル確認 | まず確認結果を記録し、修正は別タスクに分ける |
...
| BRIDGE-009 | P1 | ドキュメント整合性レビュー | レビュー待ち。現在実装、マージ済み作業、未実装領域を文書へ同期する |
