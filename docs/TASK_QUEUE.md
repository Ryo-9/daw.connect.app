...
2026-09-05 時点で、初期ドキュメント（PR #2）、モックデータによる主要 UI（PR #3）、非永続インタラクション（PR #4）、実装状態とのドキュメント同期（PR #5）、テスト方針（PR #6）、基本 PR CI（PR #7）、Level 1自動テスト（PR #8）、ブランチ保護（PR #9）、PRテンプレート（PR #10）、モバイル監査（PR #11）、タップ領域調整（PR #12）は main へマージ済みです。現在の操作データはブラウザ内の一時状態で、DB、API、認証、AWSは未実装です。mainはPR経由と`Quality checks`成功がGitHub rulesetで必須化されています。BRIDGE-004では、共通のproject context、UI方向性、AI委任境界、将来の限定的auto-merge方針を文書化します。auto-mergeの設定や実装は行いません。

次に検討する候補は以下です。順序や着手日は確定事項ではなく、担当と変更範囲を確認してから選びます。

| 候補 | ID | 内容 | 依存・注意 |
| --- | --- | --- | --- |
| 1 | BRIDGE-004 | Project contextとAI委任方針 | 作業中。共通仕様、UI方向性、委任境界、将来のauto-merge安全条件を文書化する。実装・設定変更なし |
...
| BRIDGE-004 | P1 | Project contextとAI委任方針 | 作業中。統合仕様、UI方向性、Codexへ任せる範囲、将来の限定的auto-merge条件を文書化する。設定変更は行わない |
