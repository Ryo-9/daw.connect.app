...
2026-09-10 時点で、初期ドキュメント（PR #2）、モックデータによる主要 UI（PR #3）、非永続インタラクション（PR #4）、実装状態とのドキュメント同期（PR #5）、テスト方針（PR #6）、基本 PR CI（PR #7）、Level 1自動テスト（PR #8）、ブランチ保護（PR #9）、PRテンプレート（PR #10）、モバイル監査（PR #11）、タップ領域調整（PR #12）、Project ContextとAI委任方針（PR #13）、320px楽曲詳細navigation改善（PR #14）、dark visual foundation（PR #15）、mechanical visual refinement（PR #16）、制作review surface（PR #17）、楽曲作成・編集の非保存フォーム（PR #18）、mock data schema review（PR #19）、core collaboration review flow（PR #20）、core API / persistence boundary（PR #21）、Phase 2 Cloud MVP architecture plan（PR #22）は main へマージ済みです。現在の操作データはブラウザ内の一時状態で、DB、API、認証、AWS resourceは未実装です。mainはPR経由と`Quality checks`成功がGitHub rulesetで必須化されています。現在はCLOUD-002として、AWS resource作成前のaccount / environment、IaC、credential、cost、rollback境界をdocs上で整理しています。

次に検討する候補は以下です。順序や着手日は確定事項ではなく、担当と変更範囲を確認してから選びます。

| 候補 | ID | 内容 | 依存・注意 |
| --- | --- | --- | --- |
| 1 | HOST-001 | Next.js Private Alpha hosting decision | Vercel / Amplify Hosting / AWS-native等をcompatibility、cost、rollbackで比較。AWS foundationのdeploy境界と整合させる |
| 2 | CLOUD-003 | Nonprod AWS Foundation Bootstrap | CLOUD-002とHOST-001のhuman approval後だけ開始。account security、cost visibility、short-lived access、OIDC、IaC bootstrapを小さく構築 |
| 3 | AUTH-001 | Private Alpha authentication prototype | CLOUD-003後。controlled user、email verification、session、password resetを最小実装する候補 |
| 4 | CLOUD-DATA-001 | Metadata access pattern / physical design | DynamoDBのkey、index、transaction、PITR、restoreを実装前にreviewする |
...
2026年末Private Alphaへ向けた候補です。CLOUD-001は完了し、CLOUD-002でresource作成前のfoundation decisionを整理しています。後続taskはそれぞれhuman approval、専用branch、lock、PRを必要とします。
...
| CLOUD-002 | P0 | Cloud foundation / environment / IaC decision | 作業中。account分離、Region、IaC、credential、naming、cost、rollback、destroyの方針をdocs化。resource作成なし |
