...
2026-09-03 時点で、初期ドキュメント（PR #2）、モックデータによる主要 UI（PR #3）、非永続インタラクション（PR #4）、実装状態とのドキュメント同期（PR #5）、テスト方針（PR #6）は main へマージ済みです。現在の操作データはブラウザ内の一時状態で、DB、API、認証、AWS、自動テストは未実装です。BRIDGE-006 では PR ごとの `npm ci`、lint、build だけを行う CI を追加し、テストツールは導入しません。

次に検討する候補は以下です。順序や着手日は確定事項ではなく、担当と変更範囲を確認してから選びます。

| 候補 | ID | 内容 | 依存・注意 |
| --- | --- | --- | --- |
| 1 | BRIDGE-008 | 主要 UI の自動テスト追加 | BRIDGE-005 の方針に従い、Component test と Chromium smoke E2E を最小構成で導入する |
| 2 | SURFACE-015 | 主要画面のモバイル確認 | まず確認結果を記録し、修正は別タスクに分ける |
...
| BRIDGE-006 | P1 | CI で lint / build を実行 | レビュー待ち。main 向け PR ごとに `npm ci`、lint、標準 build を自動化する。自動テストは BRIDGE-008 完了後に追加する |
