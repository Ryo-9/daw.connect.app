...
2026-09-03 時点で、初期ドキュメント（PR #2）、モックデータによる主要 UI（PR #3）、非永続インタラクション（PR #4）、実装状態とのドキュメント同期（PR #5）は main へマージ済みです。現在の操作データはブラウザ内の一時状態で、DB、API、認証、AWS、自動テスト、CI は未実装です。BRIDGE-005 ではテスト方針のみを文書化しており、ツール導入とテスト実装は行いません。

次に検討する候補は以下です。順序や着手日は確定事項ではなく、担当と変更範囲を確認してから選びます。

| 候補 | ID | 内容 | 依存・注意 |
| --- | --- | --- | --- |
| 1 | BRIDGE-006 | PR ごとの lint / build CI | 最初は `npm ci`、lint、build のみ。テストチェックは BRIDGE-008 後に追加する |
| 2 | BRIDGE-008 | 主要 UI の自動テスト追加 | BRIDGE-005 の方針に従い、Component test と Chromium smoke E2E を最小構成で導入する |
| 3 | SURFACE-015 | 主要画面のモバイル確認 | まず確認結果を記録し、修正は別タスクに分ける |
...
| BRIDGE-005 | P1 | テスト方針の作成 | レビュー待ち。`TESTING.md` で Vitest、React Testing Library、Playwright と Level 1〜3 の対象を整理した。ツールは未導入 |
| BRIDGE-006 | P1 | CI で lint / build を実行 | PR ごとに `npm ci`、lint、build を自動化する。自動テストは BRIDGE-008 完了後に追加する |
| BRIDGE-007 | P1 | Issue / タスク記述テンプレート | Codex に渡せる変更範囲と完了条件を標準化する |
| BRIDGE-008 | P1 | 主要 UI のテスト追加 | `TESTING.md` に従い、楽曲検索・絞り込み、TODO、コメントの Component test と主要導線・404 の Chromium smoke E2E を優先する |
